...
// Live Meta Graph API call is paused for now (access token expired / session
// logged out, causing OAuthException 190 on every request). Facebook dashboard
// reads from the DB instead, populated via CSV import — same pipeline as
// Shopee. Re-enable the block below once a valid long-lived access token is
// configured and remove the DB-driven implementation, or keep both and switch
...
//
//         return { id: campaign.id, name: campaign.name, status: campaign.status, spend, leads, cpl, ctr };
//     });
//
...
//         summary: { spend: totalSpend, leads: totalLeads, cpl: Number(avgCpl.toFixed(2)), ctr: Number(avgCtr.toFixed(2)) },
//         campaigns,
//     };
//
...
export async function getFacebookDashboard() {
    const raw = await getCampaignDashboardData("META");
    return mapCampaignDashboard(raw, { platform: "facebook", title: "Facebook Ads" });
}
